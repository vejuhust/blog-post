...
| ver.             | times=1000       | times=5000       | times=10000      |
|:----------------:|:----------------:|:----------------:|:----------------:|
| v0               | 233.655µs        | 240.660µs        | 237.906µs        |
| v1               | 235.866µs        | 234.752µs        | 241.464µs        |
| v2               | 234.729µs        | 236.803µs        | 240.391µs        |
| v3               | 238.325µs        | 231.046µs        | 234.024µs        |
| v4               | 221.415µs        | 221.842µs        | 223.077µs        |

各个版本的平均耗时在数量上相差不多，主要是因为Base62编码过程相对耗时，省去这一过程，测得的结果如下——

| ver.             | times=100        | times=1000       | times=10000      | times=100000     |
|:----------------:|:----------------:|:----------------:|:----------------:|:----------------:|
| v0               | 7.650µs          | 7.255µs          | 7.729µs          | 7.515µs          |
| v1               | 8.599µs          | 8.703µs          | 8.731µs          | 9.152µs          |
| v2               | 7.925µs          | 8.296µs          | 8.325µs          | 8.632µs          |
| v3               | 6.804µs          | 6.927µs          | 6.924µs          | 7.253µs          |
| v4               | 1.167µs          | 1.155µs          | 1.159µs          | 1.279µs          |

可以看出在当前环境下Version 4的效率是最高的。同时也发现了性能的瓶颈在Base62编码过程，可以作为下一步的突破口。
